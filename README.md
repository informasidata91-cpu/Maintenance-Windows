...
## Deskripsi  
Skrip PowerShell yang dibuat untuk melakukan perawatan sistem Windows secara otomatis dan terstruktur. Skrip ini ditujukan bagi administrator sistem dan pengguna tingkat lanjut yang membutuhkan maintenance rutin tanpa banyak interaksi, sekaligus ingin logging rapi untuk audit dan troubleshooting. Skrip digunakan pada perangkat Windows 10/11 di mana pemeliharaan dibutuhkan, misalnya ketika update bermasalah, performa menurun, atau setelah terdeteksi korupsi komponen sistem. Skrip dijalankan saat ingin menstabilkan sistem atau sesuai jadwal pemeliharaan, karena alurnya dapat memakan waktu dan berpotensi memicu restart untuk menyelesaikan perbaikan. Alasan utamanya adalah untuk memastikan integritas file dan image Windows, mengosongkan cache/log yang mengganggu, memulihkan layanan pembaruan, mengoptimalkan disk, dan mengurangi sumber masalah jaringan sehingga sistem kembali sehat dan responsif.  
Skrip ini melakukan perawatan sistem Windows secara otomatis. Mencakup SFC, DISM 3-langkah, pembersihan log CBS/DISM, reset komponen Windows Update, perbaikan jaringan (Flush DNS \& Winsock reset), cleanup komprehensif termasuk StartComponentCleanup dan ekstra cache/log, optimasi storage (Defrag/TRIM), CHKDSK online dengan penjadwalan perbaikan saat reboot jika diperlukan, serta Windows Memory Diagnostic. Skrip melakukan auto-elevate ke Administrator, menyetel ExecutionPolicy scope proses ke Bypass, dan mencatat transcript ke C:\MaintenanceLog.txt.  
________________________________________
## Fitur Utama  
